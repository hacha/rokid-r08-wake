...
4. 「R08 Wake」を一度開き、**「Enable auto-arm at boot」**をタップ → 設定 > ユーザー補助 で **R08 Wake auto-arm** を ON。この 1 つのユーザー補助トグルが、再起動をゼロタッチで生き残らせる肝（理由は下記 [永続化の詳細](#永続化再起動対応の詳細armapp)）。

**日常運用**

- **何もしなくてよい**。再起動のたびアプリが手動操作なしで自己 arm する：システムが起動時にユーザー補助サービスを bind → それが `r08waked` を立て直す（loopback 127.0.0.1:5555 に adb 接続 → shell → deploy＆`setsid` 起動）。有効化後は UI のボタンが「Auto-arm at boot: ON」表示になる。
- 一度立ち上がれば adb 切断・自律サスペンドは生き残る。
- ユーザー補助を無効化した場合は、アプリを開く（またはボタン）で都度 arm できる。
...
- アプリ起動（`MainActivity.onCreate`）・ボタンで arm、そして再起動耐性の本命として **`ArmAccessibilityService.onServiceConnected()`**（システムが起動時に発火）でも arm（下記）。

### 起動時ゼロタッチ自動 arm＝ユーザー補助サービス（実機確認 2026-07-02）

この ROM では `BOOT_COMPLETED` は当てにならない：サードパーティアプリは再起動のたび **`stopped`** 状態に戻され、Android は **stopped パッケージにブロードキャスト／アラーム／ジョブを一切配信しない**（`FLAG_EXCLUDE_STOPPED_PACKAGES`）。アプリを開けば `stopped` は解除されるが、次の再起動で元に戻る → `BootReceiver` 単独では発火しない。

ブロードキャストのゲートを迂回して **システムが起動時に直接 bind する**唯一のコンポーネントが **AccessibilityService**（`enabled_accessibility_services` の secure 設定から bind される。bind はコンポーネントを起動し、副作用として `stopped` も解除する）。共存している R08-Access-Bridge が同じ仕組みで動いており、それがこの ROM で機能する裏付けになった。

そこで `armapp` は、何もインターセプトせず `onServiceConnected()` で `SelfArm.armLoop()` を呼ぶだけの極小 **`ArmAccessibilityService`** を同梱する。設定 > ユーザー補助 で一度「R08 Wake auto-arm」を ON にすれば、以後は再起動のたびゼロタッチで再 arm される — **実機確認済み**：アプリを一度も開かずに再起動した後、デーモンが稼働（`accessibility connected -> arming` → `armed OK`）、`stopped=false` が bind により解除されていた。`SelfArm.arm()` は `synchronized` かつ `pkill`＋再起動なので、（best-effort で同梱を続ける）`BootReceiver` が同時に発火しても冪等。

補足：以前の「ROM が `RECEIVE_BOOT_COMPLETED` をハードブロックしている」という理解は誤診だった。実際のゲートは `stopped` フラグ（`adb install -t` による `TEST_ONLY` インストールが助長）。ユーザー補助 bind で un-stop され、かつ非 test-only で install すれば `BOOT_COMPLETED` も実際に配信される。ただしもはや主経路ではない。
...
3. `armapp` をビルドしてグラスに install。`TEST_ONLY` フラグが付かないよう **`-t` なし**で install（`adb install -r app-debug.apk`）
4. 一度開いて、設定 > ユーザー補助 で **R08 Wake auto-arm** を ON（アプリのボタンが該当画面を開く）。以後は再起動のたび自己 arm する
