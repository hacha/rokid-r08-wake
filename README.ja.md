...
2. 再起動後も loopback で adb 待受を残す：
   ...
   ```
3. `armapp` をビルドしてグラスに install（信頼済み adb 鍵と `r08waked` を assets に同梱。ビルド手順・鍵の抽出は下記 [永続化の詳細](#永続化再起動対応の詳細armapp) 参照）。ビルド済み APK は `RokidR08Wake-selfarm-debug.apk`。
...
同梱物（gitignore 済・別マシンでは要再生成）：`app/src/main/assets/r08waked`（`../build.sh` の成果物）と `app/src/main/assets/adbkey.pem`（信頼済み adb 鍵）。APK: `RokidR08Wake/RokidR08Wake-selfarm-debug.apk`。
